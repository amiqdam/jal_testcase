...
- [x] 1. Setup proyek dan konfigurasi dasar
  - [x] 1.1 Buat struktur folder proyek
  ...
  - [x] 1.2 Implementasi `backend/config.py` — konfigurasi global
  ...
  - [x] 1.3 Buat `knowledge_base/campus_info.json` — data referensi kampus
...
- [x] 2. Database Layer — Models & Migrations
  - [x] 2.1 Implementasi `backend/database/connection.py` — setup koneksi database
    - Buat fungsi `get_db()` untuk SQLite connection (default) atau PostgreSQL
    - Set WAL mode untuk SQLite concurrency
    - _Persyaratan: 10.5_

  - [x] 2.2 Implementasi `backend/database/migrations.py` — schema setup
  ...
  - [x] 2.3 Implementasi `backend/models/` — Pydantic models
...
- [x] 3. Service Layer — LLM & Knowledge Base
  - [x] 3.1 Implementasi `backend/services/knowledge_base.py` — KnowledgeBase
  ...
  - [x] 3.2 Implementasi `backend/services/llm_service.py` — LLMService
  ...
  - [x] 3.3 Implementasi `backend/services/lead_service.py` — LeadService
...
- [x] 4. AI Processing Pipeline
  - [x] 4.1 Implementasi `backend/pipeline/processor.py` — MessageProcessor
  ...
  - [x] 4.2 Implementasi individual pipeline components (di dalam `backend/pipeline/`)
  ...
  - [x] 4.3 Tulis test skenario untuk MessageProcessor
...
- [x] 5. Checkpoint — Pastikan pipeline berjalan untuk semua 17 skenario
...
- [x] 6. API Layer — FastAPI Endpoints
  - [x] 6.1 Implementasi `backend/main.py` — FastAPI entry point
    - Setup FastAPI app dengan CORS middleware (allow all origins untuk dev)
    - Mount static files untuk frontend
    - Include semua routers (chat, leads, dashboard, responses, knowledge_base, scenario, admin_export/complaints)
    - Startup event: run migrations + seed data
    - _Persyaratan: 10.1_

  - [x] 6.2 Implementasi `backend/api/chat.py` — Chat endpoints
  ...
  - [x] 6.3 Implementasi `backend/api/leads.py` — Lead endpoints
  ...
  - [x] 6.4 Implementasi `backend/api/dashboard.py` — Dashboard endpoints
  ...
  - [x] 6.5 Implementasi `backend/api/responses.py` — Response management endpoints
    - `POST /api/response/{id}/approve` — approve draft response
    - `PATCH /api/response/{id}/edit` — edit draft before sending
    - _Persyaratan: 9.7_

  - [x] 6.6 Implementasi `backend/api/admin_chat.py` — Admin chat monitor & intervention endpoints
  ...
  - [x] 6.7 Implementasi `backend/api/categorization.py` — Categorization query endpoints
...
- [x] 7. Checkpoint — Pastikan semua API endpoints berfungsi
  - Jalankan `pytest backend/tests/test_api.py`
  - Test manual via curl/httpie untuk setiap endpoint

- [x] 8. Frontend — Shared Chatbot Components
  - [x] 8.1 Implementasi `frontend/style.css` — Design system
  ...
  - [x] 8.2 Implementasi `frontend/components/chat-bubble.js` — Shared chat bubble
  ...
  - [x] 8.3 Implementasi `frontend/components/quick-start-options.js` — Quick-start pre-classifier
...
- [x] 8A. Frontend — User-Facing Chatbot
  - [x] 8A.1 Implementasi `frontend/pages/chatbot.js` — User chatbot page
...
- [x] 8B. Frontend — Admin-Facing Chat Monitor & Intervention
  - [x] 8B.1 Implementasi `frontend/pages/admin-chat.js` — Admin chat monitor
  ...
  - [x] 8B.2 Implementasi `frontend/components/chat-category-table.js` — Categorization table
  ...
  - [x] 8B.3 Implementasi `frontend/components/dynamic-charts.js` — Reactive visualizations
...
- [x] 9. Frontend — Admin Dashboard (Overview + Lead Management)
  - [x] 9.1 Implementasi `frontend/pages/dashboard.js` — Main dashboard
  ...
  - [x] 9.2 Implementasi `frontend/pages/lead-detail.js` — Lead detail page
  ...
  - [x] 9.3 Implementasi `frontend/pages/analytics.js` — Analytics page

  - [x] 9.3b Implementasi `frontend/pages/database.js` — Halaman tabel database terpusat dengan filter export

  - [x] 9.3c Implementasi `frontend/pages/settings.js` — Halaman pengaturan bot scenario & Knowledge Base Editor — Analytics page
  ...
  - [x] 9.4 Implementasi dashboard components
  ...
  - [x] 9.5 Implementasi `frontend/app.js` — Main app routing
...
- [x] 10. Checkpoint — Pastikan frontend chatbot dan dashboard berfungsi
...
- [x] 11. Seed Data & Demo Scenarios
  - [x] 11.1 Implementasi `backend/database/seed.py` — seed data
  ...
  - [x] 11.2 Verifikasi 17 skenario menghasilkan output yang benar
...
- [x] 13. Checkpoint akhir — Full system verification
